...
class Options(dict):
    """Minimal options object to replace Sphinx's directive.genopt.
    Supports both attribute access (self.options.members) and
    containment checks ('members' in self.options)."""
...
class DoxygenDocumenter(Documenter):
    ...
    def __init__(self, directive, name, indent=u'', id=None, brief=False, parent=None):
        # Handle both call paths:
        # 1. From AutodocDirective: directive is a DocumenterBridge with .genopt, .env, .result
        # 2. From DoxygenAutosummary: directive is the RST directive itself
        from sphinx.ext.autodoc.directive import DocumenterBridge
        self.directive = directive

        if isinstance(directive, DocumenterBridge):
            # Called via AutodocDirective (e.g. .. autodoxymodule::)
            self.env = directive.env
            self.options = directive.genopt
        else:
            # Called from DoxygenAutosummary
            self.env = directive.state.document.settings.env
            self.options = Options()

        self.config = self.env.config
        self._current_document = getattr(self.env, 'current_document', None)
        self._events = getattr(self.env, 'events', None)
        self.name = name
        self.indent = indent
        self.modname = ''
        self.module = None
