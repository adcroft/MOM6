...
class DoxygenAutosummary(Autosummary):
    ...
    def get_items(self, names):
        ...
        for name, i in names_and_counts:
            ...
            # Initialize result buffer for documenter output
    ...
    def get_table(self, items):
        ...
        *items* is a list produced by :meth:`get_items`.
        ...
        for name, sig, summary, real_name in items:
            # Link to the generated toctree page via :doc: reference
            if 'toctree' in self.options:
                toctree_prefix = self.options['toctree'].strip()
                col1 = ':doc:`%s <%s/%s>`' % (name, toctree_prefix, name)
            else:
                kind = self.options.get('kind', 'mod')
                qualifier = 'f:' + kind
