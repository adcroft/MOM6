...
public wright_red_density, wright_red_spec_vol
...
subroutine calculate_density_scalar_wright(T, S, pressure, rho, rho_ref)
  ...
  rho = wright_red_density(T, S, pressure, rho_ref)
...
subroutine calculate_density_array_wright(T, S, pressure, rho, start, npts, rho_ref)
  ...
  rho(js:je) = wright_red_density(T(js:je), S(js:je), pressure(js:je), rho_ref)

end subroutine calculate_density_array_wright

!> In situ density of sea water using a reduced range by Wright, 1997 [kg m-3]
!!
!! Returns the in situ density of sea water (rho in [kg m-3]) from salinity (S [PSU]),
!! potential temperature (T [degC]), and pressure [Pa].  It uses the expression from
!! Wright, 1997, J. Atmos. Ocean. Tech., 14, 735-740 with the reduced range fit coefficients.
real elemental function wright_red_density(T, S, pressure, rho_ref)
  real,           intent(in) :: T        !< potential temperature relative to the surface [degC].
  real,           intent(in) :: S        !< salinity [PSU].
  real,           intent(in) :: pressure !< pressure [Pa].
  real, optional, intent(in) :: rho_ref  !< A reference density [kg m-3].
  ...
  if (present(rho_ref)) then
    ...
    wright_red_density = (pa_000 + (p_TSp - rho_ref*(p_TSp*al0 + (b0*al_TS + lam_TS)))) / &
             ( (c0 + lam_TS) + al0*(b0 + p_TSp) )
  else
    al0 = a0 + (a1*T + a2*S)
    p0 = b0 + ( b4*S + T * (b1 + (T*(b2 + b3*T) + b5*S)) )
    lambda = c0 + ( c4*S + T * (c1 + (T*(c2 + c3*T) + c5*S)) )
    wright_red_density = (pressure + p0) / (lambda + al0*(pressure + p0))
  endif

end function wright_red_density
...
subroutine calculate_spec_vol_scalar_wright(T, S, pressure, specvol, spv_ref)
  ...
  specvol = wright_red_spec_vol(T, S, pressure, spv_ref)
...
subroutine calculate_spec_vol_array_wright(T, S, pressure, specvol, start, npts, spv_ref)
  ...
  specvol(js:je) = wright_red_spec_vol(T(js:je), S(js:je), pressure(js:je), spv_ref)

end subroutine calculate_spec_vol_array_wright

!> In situ specific volume of sea water using a reduced range by Wright, 1997 [m3 kg-1]
!!
!! Returns the in situ specific volume of sea water (specvol in [m3 kg-1]) from salinity (S [PSU]),
!! potential temperature (T [degC]) and pressure [Pa].  It uses the expression from
!! Wright, 1997, J. Atmos. Ocean. Tech., 14, 735-740 with the reduced range fit coefficients.
!! If spv_ref is present, specvol is an anomaly from spv_ref.
real elemental function wright_red_spec_vol(T, S, pressure, spv_ref)
  real,           intent(in) :: T        !< potential temperature relative to the surface [degC].
  real,           intent(in) :: S        !< salinity [PSU].
  real,           intent(in) :: pressure !< pressure [Pa].
  real, optional, intent(in) :: spv_ref  !< A reference specific volume [m3 kg-1].
  ...
  if (present(spv_ref)) then
    ...
    wright_red_spec_vol = al_TS + (lambda + (a0 - spv_ref)*p_TSp) / (b0 + p_TSp)
  else
    al0 = a0 + (a1*T + a2*S)
    p0 = b0 + ( b4*S + T * (b1 + (T*(b2 + b3*T) + b5*S)) )
    lambda = c0 + ( c4*S + T * (c1 + (T*(c2 + c3*T) + c5*S)) )
    wright_red_spec_vol = al0 + lambda / (pressure + p0)
  endif

end function wright_red_spec_vol
