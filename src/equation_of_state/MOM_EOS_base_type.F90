...
contains
  ...
  !> Deferred implementation of the in-situ density as an elemental function [R ~> kg m-3]
  procedure(i_density_elem), deferred :: density_elem
  !> Deferred implementation of the in-situ density anomaly as an elemental function [R ~> kg m-3]
  procedure(i_density_anomaly_elem), deferred :: density_anomaly_elem
  !> Deferred implementation of the in-situ specific volume as an elemental function [R-1 ~> m3 kg-1]
  procedure(i_spec_vol_elem), deferred :: spec_vol_elem
  !> Deferred implementation of the in-situ specific volume anomaly as an elemental function [R-1 ~> m3 kg-1]
  ...
  !> Returns the in-situ density or density anomaly [R ~> kg m-3]
  procedure :: density_fn => a_density_fn
  !> Returns the in-situ specific volume or specific volume anomaly [R-1 ~> m3 kg-1]
  procedure :: spec_vol_fn => a_spec_vol_fn
  !> Calculates the in-situ density or density anomaly for scalar inputs [R-1 ~> m3 kg-1]
  procedure :: calculate_density_scalar => a_calculate_density_scalar
  !> Calculates the in-situ density or density anomaly for array inputs [R-1 ~> m3 kg-1]
  procedure :: calculate_density_array => a_calculate_density_array
  !> Calculates the in-situ specific volume or specific volume anomaly for scalar inputs [R-1 ~> m3 kg-1]
  procedure :: calculate_spec_vol_scalar => a_calculate_spec_vol_scalar
  !> Calculates the in-situ specific volume or specific volume anomaly for array inputs [R-1 ~> m3 kg-1]
