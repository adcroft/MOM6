...
contains
  !> Implementation of the in-situ density as an elemental function [R ~> kg m-3]
  procedure :: density_elem => density_elem_linear
  !> Implementation of the in-situ density anomaly as an elemental function [R ~> kg m-3]
  procedure :: density_anomaly_elem => density_anomaly_elem_linear
  !> Implementation of the in-situ specific volume as an elemental function [R-1 ~> m3 kg-1]
  procedure :: spec_vol_elem => spec_vol_elem_linear
  !> Implementation of the in-situ specific volume anomaly as an elemental function [R-1 ~> m3 kg-1]
...
subroutine set_params_linear(this, Rho_T0_S0, dRho_dT, dRho_dS)
  ...
  real, optional,    intent(in)    :: Rho_T0_S0 !< The density at T=0, S=0 [R ~> kg m-3]
